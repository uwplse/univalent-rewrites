...
corresponding to the eight different classes of changes in Table~\ref{fig:changes}.
%For each case study, we explain the configuration used, walk through an example, and describe lessons learned.
These case studies demonstrate the flexibility of \toolname in handling diverse changes in datatype and diverse reuse and repair scenarios.
...
to named records.
Some examples of this can be found in a branch of the proof engineer's repository. % TODO aux material %\footnote{\url{https://github.com/Ptival/saw-core-coq/tree/dump-wip}}
...
In the first part, we used \toolname with automatic configuration to repair all of the functions and proofs about the language in \lstinline{Swap.v} 
after swapping the two constructors.
...
For the second part of the change in Figure~\ref{fig:replica}, in \lstinline{add_constr.v}, we used \toolname to repair functions 
after adding the new constructor, separating out the proof obligations for the new constructor from the old terms.
...
In \lstinline{nonorn.v}, we used \toolname to support a classic example of changing inductive structure:
