...
% TODO must define earlier in paper
...
and the same type escaping the sigma type. %, in our example $\Sigma$\lstinline{(s : packed_vect T).}$\pi_l$\lstinline{ s = n }$\simeq$\lstinline{ vector T n}.
...
then repair all of the functions and proofs about the language in \lstinline{Swap.v}.
...
In all cases, with a bit of human guidance to choose the correct permutation, \toolname repaired the functions and proofs.

For the swap change, the suggested tactics worked well.
Still, the swap change alone was simple enough that \toolname would not have been necessary
...
The change in Figure~\ref{fig:replica} includes not just swapping constructors, but also adding a new
constructor to extend a programming language.
We used \toolname to repair functions for this change as well,
...
The details of this are in \lstinline{add_constr.v}.
...
This resulted in some case explosion, but was very formulaic, and points to a clear path for automation of this class of changes in the future.
...
or use the refinement framework CoqEAL~\cite{cohen:hal-01414881}
...
We used \toolname to support this change in \lstinline{constr_refactor.v}.
...
The change in \lstinline{flip.v} came at the request of a different non-author, and shows how to 
...
The configuration for \lstinline{N} used functions from the Coq standard library
for \lstinline{DepConstr} and \lstinline{DepElim}. 
...
By writing a manual configuration with nontrivial \lstinline{Iota}, it was possible for us to implement this transformation 
that had previously been its own tool.
...
After annotating, we obtained automatically repaired proofs about the slow version of addition,
which we found simple to port to fast binary addition.
We hope to automate this last step in the future using CoqEAL~\cite{cohen:hal-01414881}.
Repaired tactics were partially useful, but struggled with making sense of custom eliminators, and with generating useful
