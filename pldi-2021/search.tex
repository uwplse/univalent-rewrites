...
In file~\circled{3}, we used this to port
...
In the same file~\circled{3}, we went on to port functions and proofs to vectors of a \textit{particular} length, 
...
to named records, as in file~\circled{18}. % TODO aux material %\footnote{\url{https://github.com/Ptival/saw-core-coq/tree/dump-wip}} TODO note in guide it got a bit abandoned because proof engineer got stuck in france...
...
after swapping the constructors~\circled{1}.
...
The change in \circled{20} came at the request of a different non-author (a cubical type theory expert),
...
updatings unary to binary numbers~\cite{magaud2000changing}.
