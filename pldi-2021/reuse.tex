...
Given an equivalence between two types \A and \B,
\toolname repairs functions and proofs defined over \A to instead refer to \B.
It does this in a way that allows for removing all references to \A, which is essential for proof repair,
...
All of the changes in Table~\ref{fig:changes} in Section~\ref{sec:search} can be described by equivalences.
To give some more intuition for what kinds of changes can be described this way, we preview two changes below.
...
The file \lstinline{constr\_refactor.v} in auxiliary material
...
\lstinline{Example.v} in auxiliary material shows how to use \toolname to repair functions and proofs
about lists to functions and proofs about vectors of a particular length, since:
...
\end{lstlisting}
by \lstinline{app_ok} in \lstinline{Swap.v}.
...
\toolname implements transport in a way that allows for removing references to the old specification.
...
Section~\ref{sec:configurable} explains a configuration in more detial, and
Section~\ref{sec:art} shows that every equivalence
