...
which we will define formally in Section~\ref{sec:configurable}.
...
Every equivalence induces a configuration (\href{https://github.com/uwplse/pumpkin-pi/blob/master/plugin/coq/playground/arbitrary.v}{arbitrary.v}),
though not necessarily one that is suitable for repair (removing references to the old type) as opposed to just reuse.
