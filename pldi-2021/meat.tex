...
proofs across equivalences~\circled{4}. % lift.ml
...
then finally specifies correctness criteria for the configuration (Section~\ref{sec:art}).
Section~\ref{sec:implementation} describes the additional work needed to go from this transformation to the implementation.
...
At a high level, the configuration helps the transformation achieve two goals: preserve equality up to transport along the equivalence 
...
\lstinline{DepConstr} and \lstinline{DepElim} define how to transform constructors and eliminators, thereby preserving the equivalence (Section~\ref{sec:equivalence}), and 
\lstinline{Eta} and \lstinline{Iota} define how to transform $\eta$-expansion and $\iota$-reduction of constructors and eliminators, thereby producing well-typed terms (Section~\ref{sec:equality}).
...
\begin{lstlisting}
packed_vect T := $\Sigma$(n : nat).vector T n.(@\vspace{-0.04cm}@)
\end{lstlisting}
This type is equivalent to \lstinline{list T}, and so \toolname can port proofs along that equivalence~\circled{3}. % Example.v
...
DepConstr(0, packed_vect) : packed_vect T :=(@\vspace{-0.04cm}@)
  $\exists$ (Constr(0, nat)) (Constr(0, vector T)).(@\vspace{-0.04cm}@)
\end{lstlisting}
...
\begin{lstlisting}
Eta(packed_vect) := $\lambda$(s:packed_vect).$\exists$ ($\pi_l$ s) ($\pi_r$ s).(@\vspace{-0.04cm}@)
\end{lstlisting}
...
between them~\circled{5}, % nonorn.v
...
$\forall$ P p$_\texttt{0}$ p$_\texttt{S}$ n,(@\vspace{-0.04cm}@)
  ...
  (@\codediff{p$_\texttt{S}$}@) n (DepElim(n, P) { p$_\texttt{0}$ p$_\texttt{S}$ }).(@\vspace{-0.04cm}@)
\end{lstlisting}
...
The \lstinline{Iota} rules are rewrites along proofs of the theorems above for the successor case~\circled{5}.
...
and with applications of the configuration terms over \A. 
The transformation then transforms the applications of the configuration terms over \A
to applications of the configuration terms over \B.
...
that unify subterms with applications of configuration terms, and that instantiate parameters and dependent indices in those configuration subterms~\circled{6}.
...
Recall from Section~\ref{sec:repair} that the goal of the transformation is to preserve equality up to transport along the equivalence $A \simeq B$,
...
Further recall from Section~\ref{sec:repair} that this goal for correctness is metatheoretical,
though it can be approximated inside of Coq in some particular cases~\cite{tabareau2017equivalences}.
%, as
%stating and proving equality up to transport is in general not possible in Coq
%without additional axioms, though it is in some particular cases realizable~\cite{tabareau2017equivalences}.
...
the proof engineer must vet the transformed specifications to ensure that they specify the expected behavior.
...
change, only some of which are useful (for example~\circled{7}, any \A is equivalent to \lstinline{unit} refined by \A). % refine_unit.v
...
dep_elim_ok : $\forall$ a b (P : A $\rightarrow$ Type) (Q : B $\rightarrow$ Type), a $\equiv_{A \simeq B}$ b $\rightarrow$ P $\equiv_{A \simeq B}$ Q $\rightarrow$ DepElim(a, P) $\equiv_{A \simeq B}$ DepElim(b, Q).(@\vspace{-0.04cm}@)
(@\vspace{-0.14cm}@)
(* --- Equality (shown over A; B is similar) --- *)(@\vspace{-0.04cm}@)
...
That is, \lstinline{section} and \lstinline{retraction} must hold for functions \lstinline{f} and \lstinline{g},
where \lstinline{f} uses \lstinline{DepElim} to eliminate \A and \lstinline{DepConstr} to construct \B, and \lstinline{g} is similar.
...
With this correctness criteria for a configuration, we get the completeness result (proven in Coq~\circled{8}) that every equivalence induces a configuration. % arbitrary.v
...
that correspond to automatic configuration~\circled{9}, % search.ml
and also generate proofs \lstinline{section} and \lstinline{retraction} that these functions form an equivalence~\circled{10}. % equivalence.ml
...
If the proof engineer wishes, it is possible to prove these in individual cases~\circled{8}, % arbitray.v
but the proof engineer does not need to prove these to use \toolname---they simply need to hold.
