...
that unify subterms with applications of configuration terms, and that instantiate parameters and dependent indices in those subterms~\circled{6}. % liftconfig.ml
...
\toolname implements four search procedures for automatic configuration~\circled{6};
due to space constraints, we do not discuss these search procedures in detail.
The algorithm above is essentially what \textbf{Configure} uses to generate functions \lstinline{f} and \lstinline{g} for these configurations~\circled{9}, % search.ml
