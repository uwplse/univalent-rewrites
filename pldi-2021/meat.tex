...
The corresponding $\iota$ rule for \lstinline{N} is propositional, since:
...
The \lstinline{Iota} rules are exactly rewrites along proofs of the theorems above for the successor case~\circled{5}.
...
\end{mathpar}
\vspace{-0.3cm}
\caption{Transformation for transporting proof terms across an equivalence $A \simeq B$ described by configuration \lstinline{((DepConstr, DepElim), (Eta, Iota))}.}
\label{fig:final}
\end{figure*}
...
Note that this goal for correctness is metatheoretical, as
...
given change in specification, only some of which are useful (for example~\circled{7}, any \A is equivalent to \lstinline{unit} refined by \A). % refine_unit.v
...
Once a configuration has been chosen, we can specify what it means for that configuration to be correct.
