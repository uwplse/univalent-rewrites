...
With this correctness criteria for a configuration, we get the completeness result that every equivalence induces a configuration (Section~\ref{sec:completeness}).
We also obtain an algorithm for the soundness result that every configuration induces an equivalence (Section~\ref{sec:soundness}),
which we use in particular cases to produce correctness proofs in \toolname, but do not yet prove in the general case.
We do not yet know if configurations and equivalences are themselves equivalent---we leave that to future work.

\subsubsection{Completeness}
\label{sec:completeness}

TODO explain, link to proof, update proof to show whatever results we can show
