...
In a loop, the proof engineer passes Coq high-level search procedures called \textit{tactics} (like \lstinline{induction}),
and Coq responds to each tactic by refining the current goal to some subgoal (like the proof obligation for the base case).
This loop of tactics and subgoals continues until no subgoals remain, at which point the proof engineer 
has constructed a sequence of tactics called a \textit{proof script}.
To check this proof script for correctness, Coq compiles it down to a low-level function called a \textit{proof term},
