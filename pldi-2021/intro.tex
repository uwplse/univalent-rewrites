...
The problem of automatically updating proofs in response to changes in programs or specifications is known as \textit{proof repair}.
...
This is implemented (Section~\ref{sec:impl}) in \toolname\footnote{Name changed for double-blind.} (Configurable Approach to Repairing \& Refactoring Outdated Tactics), a proof repair plugin for Coq 8.8 that is available on Github.\footnote{For double-blind, we annotate each claim to which the code is relevant with a circled number like \circled{1}. The auxiliary material maps these to code.}
