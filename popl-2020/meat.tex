...
\lstinline{Eta} describes how to $\eta$-expand an \A in a way that is coherent with the dependent eliminator \lstinline{DepElim}.
This describes $\eta$-expansion for \textit{propositional} $\eta$-conversion,
(constructing and eliminating an \A in a way that preserves propositional but not definitional equality),
which is important for defining dependent eliminators~\cite{nlab:eta-conversion}.
For example, for the change from list to \lstinline{$\Sigma$(n : nat).vector T n}, this corresponds to $\eta$-expansion
of $\Sigma$ types:
...
It is also parameterized over the configuration (\lstinline{DepConstr}, \lstinline{DepElim}, \lstinline{Eta}, and \lstinline{Iota}),
...
The transformation is simple because it moves the bulk of the work into the configuration, and into unifying input terms with 
the configuration. Before running the transformation, \toolname unifies the input proof term with applications of the 
terms in the configuration for \A. The transformation then transforms those to applications to the terms in the configuration for \B.
...
\textbf{Configure} implements search procedures that discover the functions \lstinline{f} and \lstinline{g} for certain classes of
...
This can be challenging, so we plan to let proof engineers write
...
by breaking them into parts and reasoning seperately about the projections.
