...
Each \lstinline{Iota} needs to prove and rewrite along the simplification (formally, \textit{refolding}~\cite{boutillier:tel-01054723}) behavior that corresponds to a case of the dependent eliminator, in other words: % TODO do we need eta here?
...
Each of our search procedures for automatic configuration in turn implements custom unification heuristics (\href{https://github.com/uwplse/pumpkin-pi/blob/master/plugin/src/automation/lift/liftconfig.ml}{liftconfig.ml}) that tell \toolname
