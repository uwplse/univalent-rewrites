...
and the result is configurable both by the writer of the proof reuse tool and by the 
proof engineer (Section~\ref{sec:configurable}).
\end{quote}
...
for our refactored list append function \lstinline{++} from Section~\ref{sec:overview},
...
\lstinline{Eta} describes how to $\eta$-expand the identity function in a way that preserves equalities
...
Section~\ref{sec:search} describes more examples of both \lstinline{Eta} and \lstinline{Iota}, and elaborates on this example.
...
up to transport, for example, requires either a special framework~\cite{tabareau2017equivalences} % TODO cite UP
