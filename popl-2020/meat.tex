...
For example, if our input type is \{\lstinline{ n : nat | n <> O }\}, we could choose these dependent constructors:

\begin{lstlisting}
(* TODO may change this example---not sure if DepElim is possible to define for this *)
DepConstr(0, { n : nat | n <> O }) : { n : nat | n <> O } :=
  $\exists$ 1 (not_eq_sym (n_Sn O)).

DepConstr(1, { n : nat | n <> O }) : { n : nat | n <> O } -> { n : nat | n <> O } :=
  fun s => $\exists$ (S (proj1_sig s)) (not_eq_sym (O_S (proj1_sig s))).
\end{lstlisting} % TODO test this example w/ lifting
% TODO what is the eliminator? Is this possible? If not, mention we don't know if always possible,
% and change example. If so, include it here.
...
structure---they are not \textit{ornaments}. % TODO cite
