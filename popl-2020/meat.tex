...
For example, for our \lstinline{positive} example, we could just as well
map \lstinline{O} over natural numbers to and from \lstinline{1} over positive numbers,
and preserve the successor behavior, producing this equivalence: % TODO could we though?

\begin{lstlisting}
nat $\simeq$ positive
\end{lstlisting}
Transporting functions and proofs along this equivalence is a simple refactoring,
but it does not preserve functions and proofs with the desired behavior.
And even once we've chosen an equivalence, we could define many possible configurations that correspond
