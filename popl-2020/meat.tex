...
DepConstr(<i>, <t>) | DepElim(<t>, <t>)\{<t>,\ldots,<t>\} | Eta (<t>) | Iota(<i>, <t>, <t>, <t>)
\end{grammar}
...
\item \lstinline{Eta} and \lstinline{Iota} to configure the transformation to reason about equalities.
\end{enumerate}
...
\item \lstinline{Eta} and \lstinline{Iota} define how to transform identity and equalities, thereby producing well-typed terms (Section~\ref{sec:equality}).
\end{enumerate}
...
which use configuration parts \lstinline{Eta} (\textit{eta-expanded identity}) and \lstinline{Iota} (\textit{iota reduction}, or refolding cases of eliminators), respectively.
These together describe the essence of identity and equality as they relate to \lstinline{DepConstr}
and \lstinline{DepElim}.

\lstinline{Eta} describes how to expand the identity function in a way that preserves equalities
...
Each \lstinline{Iota}---one per constructor---describes and proves the refolding behavior
...
\lstinline{Iota} simply describes how to do this.
For both \lstinline{nat} and \lstinline{N}, each \lstinline{Iota} in the successor case:
...
\lstinline{Iota} then lets us transform that propositional equality to a propositional equality over \lstinline{N},
which gives us a proof of the theorem we want.

\lstinline{Iota(N)} in the above example is not reflexivity precisely because there is no ornament
...
each \lstinline{Iota} just rewrites by reflexivity.
...
\lstinline{Eta} and \lstinline{Iota}. Since constructors and eliminators are dual,
...
Each \lstinline{Iota} needs to prove and rewrite along the refolding behavior that corresponds to a case of the dependent eliminator, in other words:
