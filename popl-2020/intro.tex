...
\begin{enumerate}
\item supports changes informed by real user needs and not supported by other repair tools, and
...
\begin{enumerate}
\item has helped an industrial proof engineer integrate Coq with a company workflow, port tuples to records, and write proofs about an implementation of the TLS Handshake Protocol,
\item can help proof engineers port proofs from non-dependent types to certain dependent types,
\item can rename and swap constructors to support variants of a benchmark from a user study of Coq proof engineers, and
