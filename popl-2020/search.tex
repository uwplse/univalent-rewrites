...
\end{enumerate}
For each case study, we explain the configuration used, provide example proofs, and describe experiences using \toolname.
...
Some user workflows were counterintuitive to us and informed changes in the design of \toolname.
\end{enumerate}
...
However, they had found that the generated specifications were sometimes too difficult to work with.

A proof engineer at \company has used \toolname to work with those automatically generated specifications
with the following workflow:

\begin{enumerate}
\item First, the proof engineer uses \toolname to refactor the automatically generated specifications into more
human-readable specifications.
\item Next, the proof engineer writes Coq proofs about the more human-readable specifications.
\item Finally, the proof uses \toolname again to refactor those proofs about human-readable specifications back to
proofs about the original automatically generated specifications.
\end{enumerate}
...
notes about challenges and so on.
In the initial days, we worked closely with the proof engineer to watch interaction with \toolname and understand
improvements to \toolname that would help the proof engineer.
Later, the proof engineer worked independently and reached out occasional by email.

Usability was simple enough for the proof engineer to work independently from another country with some contact by email.
However, the user did sometimes struggle with understanding errors (which informed significant changes to our error messaging)
and with understanding the behavior of \toolname when 
There were some challenges in scale, many of which inspired
changes to \toolname that have already been implemented, and some of which remain.
line blurry here, what really is this? refactoring i guess but not in a standard way
tactic workflow was not needed here, but other workflow integration helped

each criteria though:
\begin{enumerate}
