...
\begin{enumerate}
\item an industrial proof engineer using \toolname in verifying an implementation of the TLS handshake protocol (automatic, Section~\ref{sec:industry}),
...
\item supporting a benchmark from a user study of Coq proof engineers (automatic, Section~\ref{sec:replica}), and
...
but also to repair some non-dependently-typed
functions and proofs to instead use dependent types.
In total, this used at least three automatic configurations and no manual configurations.
...
Both of these observations informed significant changes to \toolname, like better error messages, caching of transformed subterms,
...
% TODO both should take the same number of arguments, even ifeta_A takes fewer. also does this belong in rew_eta?
...
functions that carry proofs about length invariants. % TODO test again
