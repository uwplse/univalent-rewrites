...
\subsection{Repair Across Equivalences (Transport with a Twist)}
\label{sec:repair}
...
The changes in response to which \toolname repairs proofs correspond to
\textit{type equivalences}~\cite{univalent2013homotopy},
...
\subsection{\textsc{Carrot}: A Tool for Repair Across Equivalences}
\label{sec:time}
