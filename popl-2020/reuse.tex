...
\begin{quote}
\textbf{Insight 1}:
Proof repair is a form of proof reuse, with the additional
challenge that the specification about which
we are reusing proofs ceases to exist (Section~\ref{sec:repair}).
...
\subsection{Repair as Transport with a Twist}
\label{sec:repair}
...
The goal of a proof repair tool like \toolname is thus to define a transport method that
...
and in all of its dependencies.
...
Viewing repair as transport supports not just simple refactorings,
