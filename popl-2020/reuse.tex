...
The trick is that, in that case, \toolname repairs proofs along an equivalence between
between \textit{refinements} ($\Sigma$ types).
...
The key to handling changes that add or remove information usefully is to separate the new information from redundant old information, 
...
Formally, the scope of changes that \toolname can handle is all changes that can be described by a configuration that
we will define in Section~\ref{sec:configurable}.
This does not solve the problem of determining when \toolname can handle changes \textit{usefully}---for that,
we rely on examples, feedback from proof engineers, and case studies.
Section~\ref{sec:search} shows some case studies using \toolname to repair real proofs
